...
const MANUAL_REPORT_SECTION_OPTIONS: Array<{
  id: ReportSection;
  label: string;
}> = [
...
export default function LiveAuditResults({
  report,
  manualMode = false,
}: {
  report: AuditReport;
  ...
  const [reportMode, setReportMode] = useState<"manual" | "automatic">(
    "manual",
  );
  const [manualReportSections, setManualReportSections] = useState<
    ReportSection[]
  >(MANUAL_REPORT_DEFAULT_SELECTION);
  ...
  const competitorAvg = report.competitors?.length
    ? Math.round(
        report.competitors.reduce((sum, c) => sum + c.score, 0) /
          report.competitors.length,
      )
    : null;

  const failedStages = (report.stageTrace || []).filter(
    (s) => s.status === "failed",
  );
  const pipelineWarnings = report.pipelineWarnings || [];

  return (
    <div className="mt-8 space-y-8">
      {/* Top Overview Section */}
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="liquid-glass-soft lg:col-span-2">
          <CardContent className="flex flex-col items-center justify-between gap-6 p-6 md:flex-row">
            <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 min-w-0 flex-1 w-full md:w-auto text-center sm:text-left">
              ...
              <div className="min-w-0 flex-1 w-full max-w-[280px] sm:max-w-none">
                <div className="flex items-center justify-center sm:justify-start gap-2">
                  ...
                  <p className="font-mono text-sm text-emerald-100/65 truncate">
                    Target URL
                  </p>
                </div>
                <h2
                  className="mt-1 text-xl sm:text-2xl font-bold tracking-tight text-emerald-50 truncate"
                  title={report.url}
                >
                  {report.url}
                </h2>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 shrink-0 flex-wrap justify-center">
              <div className="flex flex-col items-center gap-3">
                ...
                <button
                  onClick={
                    mockupStatus === "idle" || mockupStatus === "error"
                      ? generateMockup
                      : undefined
                  }
                  disabled={mockupStatus === "loading"}
                  className="flex w-full sm:w-auto items-center justify-center gap-2 rounded-xl bg-emerald-300 px-4 py-2 text-sm font-semibold text-black shadow-lg shadow-emerald-500/25 transition-all hover:bg-emerald-200 disabled:opacity-50"
                ...
                {mockupStatus === "error" && (
                  <span className="text-xs text-red-500 font-medium animate-pulse">
                    Failed to generate preview. Try again.
                  </span>
                )}
              </div>
            </div>
        ...
        <div className="space-y-8 lg:col-span-2">
          <div>
            <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="text-xl font-semibold tracking-tight text-emerald-50">
                  Core Diagnostics
                </h3>
              </div>
              <div className="flex flex-wrap gap-2">
                {report.uxIssuesCount !== undefined && (
                  <Badge
                    variant="outline"
                    className="border-emerald-300/40 text-emerald-200 bg-emerald-500/10"
                  >
            ...
            <AuditCards
              ...
              industryAverageOverall={competitorAvg ?? undefined}
            />
          </div>
          ...
          {manualMode && (
            <>
                ...
                <div className="relative overflow-hidden rounded-2xl border border-emerald-500/30 bg-gradient-to-r from-emerald-950 via-teal-950 to-emerald-950 p-5 shadow-xl shadow-emerald-500/10">
                  <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-4">
                    <div className="flex-1">
                      <p className="text-xs font-black tracking-widest text-emerald-400 uppercase mb-1">
                        Lead Gen Score
                      </p>
                      ...
                      <p className="mt-1 text-sm text-emerald-200/60">
                        {report.leadGenAnalysis.aboveFoldCta
                          ? "✓ CTA above fold"
                          : "✗ No above-fold CTA"}{" "}
                        ·{" "}
                        {report.leadGenAnalysis.hasContactForm
                          ? "✓ Contact form"
                          : "✗ No contact form"}
                      </p>
                    </div>
                    <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/20 px-4 py-3 text-center">
                      ...
                      <p className="text-2xl font-black text-white mt-1">
                        {report.leadGenAnalysis.roiImpact}
                      </p>
              ...
              <ScoreTable
                uxIssues={
                  report.manualRulesIssues?.filter(
                    (_, i) => i < (report.uxIssuesCount ?? 0),
                  ) ?? []
                }
              ...
              <div className="relative z-10 flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-black tracking-widest text-white uppercase drop-shadow-md">
                      Pixel-Perfect CV Analysis
                    </span>
                  </div>
                  <Badge className="bg-white/20 text-white hover:bg-white/30 border-none text-[10px] backdrop-blur-sm">
                    OpenCV Engine
                  </Badge>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                  {[
                    ...
                    <div
                      key={stat.label}
                      className="flex flex-col rounded-lg bg-white/10 p-2 backdrop-blur-md border border-white/10"
                    >
                      <span className="text-[10px] font-bold text-white/70 uppercase leading-none mb-1">
                        {stat.label}
                      </span>
                      <span className="text-lg font-black text-white leading-none">
                        {stat.value}
                        <span className="text-[10px] font-normal opacity-50 ml-0.5">
                          pts
                        </span>
                      </span>
                    </div>
          ...
          {report.aiStatus === "failed" && (
            <div className="flex items-center gap-2 rounded-xl bg-orange-500/10 p-3 text-xs text-orange-400 border border-orange-500/20 mb-4">
              ...
              <span>
                AI Analysis in fallback mode. Results are based on deterministic
                heuristics.
              </span>
            </div>
          )}
          ...
          <Card className="liquid-glass-soft">
            ...
            <CardContent className="flex flex-wrap items-center gap-4">
              ...
              {report.roiSource && <ROISourceBadge source={report.roiSource} />}
            </CardContent>
          </Card>
              ...
              {reportMode === "manual" ? (
                <div className="space-y-3 rounded-xl border border-border bg-secondary/20 p-4">
                  <p className="text-sm text-foreground/80">
                    Select exactly what you want to include in the report.
                  </p>
                  ...
                  <div>
                    <PDFReport
                      ...
                      buttonLabel="Generate Clear Manual Report"
                      ...
                      payload={{
                        ...
                        summary:
                          report.summary ||
                          report.aiInsights?.issues?.[0]?.fix ||
                          "",
                ...
                <div className="space-y-3 rounded-xl border border-border bg-emerald-500/5 p-4">
                  <p className="text-sm text-foreground/80">
                    This mode creates a plain-language report for non-technical
                    users with simple explanation, top issues, fix plan, trust,
                    and ROI.
                  </p>
                  <div>
                    <PDFReport
                      mode="automatic"
                      buttonLabel="Generate Clear Automatic Report"
              ...
              {sortedRecommendations.length ? (
                <div className="grid gap-4 md:grid-cols-2">
                  {sortedRecommendations.map((item, idx) => (
                    <div
                      key={`${item.category}-${item.action}-${idx}`}
                      className="flex flex-col rounded-xl border border-border bg-secondary/20 p-4 transition-colors hover:bg-secondary/40"
                    >
              ...
              <CardContent>
                <div className="grid gap-4 sm:grid-cols-3">
                  {report.quick_wins.map((win, idx) => (
                    <div
                      key={idx}
                      className="flex flex-col rounded-xl border border-success/30 bg-background/50 p-4 transition-all hover:border-success/60 shadow-sm"
                    >
                      <div className="mb-2 flex items-center justify-between">
                        ...
                        <span className="text-[9px] font-black text-success uppercase leading-none">
                          {win.impact}
                        </span>
                      </div>
        ...
        <div className="space-y-8">
          <div className="space-y-2">
            <div className="flex items-center justify-between px-1">
              <h3 className="text-sm font-medium text-emerald-100/70">
                Score radar
              </h3>
            </div>
          ...
          <Card className="liquid-glass-soft">
            <CardHeader className="pb-3">
              <CardTitle className="flex flex-wrap items-center gap-2 text-lg text-emerald-100">
            ...
            <CardContent className="space-y-4">
              {report.contentFixes?.length ? (
                report.contentFixes.slice(0, 3).map((fix) => (
                  <div
                    key={`${fix.type}-${fix.suggested}`}
                    className="rounded-xl border border-emerald-300/20 bg-black/35 p-4"
                  >
                    <div className="mb-3 flex items-center justify-between">
                      <Badge
                        variant="outline"
                        className="border-emerald-300/35 text-emerald-200"
                      >
                        {contentFixLabel(fix.type)}
                      </Badge>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="rounded-md border border-rose-300/20 bg-rose-500/5 p-3">
                        <span className="mb-1 block text-[10px] font-bold tracking-wider text-destructive uppercase">
                          Before
                        </span>
                        <p className="text-xs text-emerald-100/55 line-through">
                          {fix.current || "Missing"}
                        </p>
                      </div>
                      <div className="rounded-md border border-emerald-300/20 bg-emerald-500/10 p-3">
                        <span className="mb-1 block text-[10px] font-bold tracking-wider text-emerald-300 uppercase">
                          After
                        </span>
                ...
                <p className="py-4 text-center text-sm text-emerald-100/60">
                  No critical content issues detected.
                </p>
              )}
            </CardContent>
          </Card>
