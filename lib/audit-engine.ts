...
function isErrorLikeText(value: string): boolean {
  ...
  return /(^\s*(error[:-]|403|404|429|500|502|503)\b|\b\d{3}\s+error\b|request could not be satisfied|generated by cloudfront|access denied|\bforbidden\b|service unavailable|temporarily unavailable|\bnot found\b)/i.test(
    t
  );
}
...
export function generateContentSuggestions(htmlRaw: string, industry: string, industryConfidence: number): ContentSuggestion[] {
  ...
  if (errorPageDetected) {
    ...
    if (useIndustryInCopy) {
      ...
      h1Pack = {
        suggested: `Grow ${industry} results with a clearer, faster website`,
