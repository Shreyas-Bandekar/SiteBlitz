...
export type ShlokLocationSignals = {
  city?: string;
  district?: string;
  state?: string;
  country?: string;
...
export function detectShlokLocationSignals(htmlRaw: string, pageUrl: string): ShlokLocationSignals {
  ...
  if (!city || !state || !country) {
    const m = text.match(
      /([A-Za-z ]+),\s*([A-Za-z ]+),\s*(India|United States|United Kingdom|Canada|Australia|Singapore|UAE)/i
    );
    if (m) {
      city = city || clean(m[1]);
